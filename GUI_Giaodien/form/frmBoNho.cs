...
namespace GUI_Giaodien.form
{
    public partial class frmBoNho : Form
    {
                ...
                if (row == dataGrid.Columns["Ok"].Index)
                {
                    ...
                    if (a == "Add")
                    {
                        ...
                        if (MessageBox.Show("Bạn có chắc thêm không", mess, MessageBoxButtons.OKCancel) == DialogResult.OK)
                        {
                            ...
                            _bonho.Name = dataGrid.Rows[rowindex].Cells[1].Value.ToString();
                            
                            MessageBox.Show(sp.AddBN(_bonho), mess);
                            loadDB();
                        }
                    }
                    if (a == "Update")
                    {
                        if (sp.selectBonhoUP(IDwhenClick) == null) return;
                        else
                        {

                            var sprs = sp.selectBonhoUP(IDwhenClick);
                            ...
                            {
                                MessageBox.Show(sp.UpdateBN(sprs), mess);
                                loadDB();
                            }
                        }
                    }
                    if (a == "Delete")
                    {
                        ...
                        {
                            MessageBox.Show(sp.DeleteBN(IDwhenClick), mess);
                            loadDB();
                        }
                    }
                }
        ...
        private void dataGrid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (rowindex == sp.GetlistBonhos().Count || rowindex == -1) return;
        ...
        {
            dataGrid.Rows.Clear();
            foreach (var x in sp.GetlistBonhos().Where(c=>c.Name.Contains(txt_find.Text)))
            {
                dataGrid.Rows.Add(x.Id, x.Name);
            }
        }
